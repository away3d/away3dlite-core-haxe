...
class Mesh extends Object3D
{
	...
	/*arcane*/ private override function project(camera:Camera3D, ?parentSceneMatrix3D:Matrix3D):Void
	{
		...
		if (stage == null)
			stage = jsflash.Manager.stage;
		
		this.material.renderMesh(this, camera, stage);
	...
	**/
	#if !flash9
	/*public var stData(default, null):webgl.WebGLArray.WebGLFloatArray;
	public var verticesData(default, null):webgl.WebGLArray.WebGLFloatArray;
	public var indicesData(default, null):webgl.WebGLArray.WebGLFloatArray;*/
	public var stBuffer(default, null):webgl.wrappers.WebGLObject.WebGLBuffer;
	public var verticesBuffer(default, null):webgl.wrappers.WebGLObject.WebGLBuffer;
	public var indicesBuffer(default, null):webgl.wrappers.WebGLObject.WebGLBuffer;
	...
	/*arcane*/ private function buildFaces():Void
	{
		var gl =if (stage != null)
			stage.RenderingContext;
		else
			jsflash.Manager.stage.RenderingContext;
		_indicesTotal = 0;
		
		var len:Int = _faceLengths.length;
		if (len == 0)
			len = Std.int(_indices.length / 3);
		var index:Int = _indices.length;
		var faceLength:Int;
		...
		while (++i < len) {
			...
			if (faceLength == 3)
			{
				_iData.push3(_indices[idx++], _indices[idx++], _indices[idx++]);
				_indicesTotal += 3;
				numItems++;
			}
			else if (faceLength == 4)
			{
				...
				var i1 = _indices[idx++];
				var i2 = _indices[idx++];
				_iData.push3(i0, i1, i2);
				_iData.push3(i0, i2, _indices[idx++]);
				_indicesTotal += 6;
				numItems += 2;
			} else {
				_iData.push3(_indices[idx++], _indices[idx++], _indices[idx++]);
		...
		numItems = _indicesTotal;
		
		var len = _uvtData.length;
		i = -1;
		while (++i <  len)
		{
			_stData.push(_uvtData[i]);
			_stData.push(_uvtData[++i]);
			i++;
		}
		//TODO: can we reuse the buffers?
		//if (indicesBuffer == null)
			indicesBuffer = gl.createBuffer();
		gl.bindBuffer(GLEnum.ELEMENT_ARRAY_BUFFER, indicesBuffer);
		
		gl.bufferData_2(GLEnum.ELEMENT_ARRAY_BUFFER, new webgl.WebGLArray.WebGLUnsignedShortArray(_iData), GLEnum.STATIC_DRAW);
		
		//if (stBuffer == null)
			stBuffer = gl.createBuffer();
		gl.bindBuffer(GLEnum.ARRAY_BUFFER, stBuffer);
		
		gl.bufferData_2(GLEnum.ARRAY_BUFFER, new webgl.WebGLArray.WebGLFloatArray(_stData), GLEnum.STATIC_DRAW);
		
		//if (verticesBuffer == null)
			verticesBuffer = gl.createBuffer();
		gl.bindBuffer(GLEnum.ARRAY_BUFFER, verticesBuffer);
		
		gl.bufferData_2(GLEnum.ARRAY_BUFFER, new webgl.WebGLArray.WebGLFloatArray(_vertices), GLEnum.STATIC_DRAW);
