...
import jsflash.display.Graphics;
import jsflash.geom.Matrix;
import jsflash.geom.Rectangle;
import jsflash.display.InteractiveObject;
import jsflash.text.FontInstance;
import jsflash.display.DisplayObject;
import jsflash.text.FontManager;
import jsflash.text.TextFormatAlign;
import jsflash.events.Event;
import jsflash.events.KeyboardEvent;
import jsflash.events.FocusEvent;
import jsflash.text.KeyCode;
...
class jsflashText extends jsflash.display.InteractiveObject
{
   ...
   static var sSelectionOwner:jsflashText = null;
   ...
   public function new()
   {
      ...
      mFace = mDefaultFont;
      mAlign = jsflash.text.TextFormatAlign.LEFT;
      ...
      mType = jsflash.text.TextFieldType.DYNAMIC;
      autoSize = jsflash.text.TextFieldAutoSize.NONE;
      mTextHeight = 12;
   ...
   override public function GetGraphics() : jsflash.display.Graphics { return mGraphics; }
   ...
   override public function OnMouseDown(inX:Int, inY:Int)
   {
      if (tabEnabled || selectable)
      {
         ...
         var pos = globalToLocal( new jsflash.geom.Point(gx,gy) );
   ...
   function RenderRow(inRow:RowChars, inY:Int, inCharIdx:Int,inAlign:String, ?inInsert:Int) : Int
   {
      ...
      if (inInsert!=null)
      {
         if (autoSize != jsflash.text.TextFieldAutoSize.NONE)
         {
            mScrollH = 0;
            insert_x = inInsert;
         }
      ...
      if (autoSize == jsflash.text.TextFieldAutoSize.NONE && w<=mLimitRenderX)
      {
   ...
   function Rebuild()
   {
      ...
      mMaxWidth = 0;
      mLimitRenderX = (autoSize == jsflash.text.TextFieldAutoSize.NONE) ? Std.int(width) : 999999;
      ...
      switch(autoSize)
      {
         case jsflash.text.TextFieldAutoSize.LEFT:
            width = w;
            height = h;
         case jsflash.text.TextFieldAutoSize.RIGHT:
            var x0 = x + width;
            width = w;
            height = h;
            x = x0 - w;
         case jsflash.text.TextFieldAutoSize.CENTER:
            var x0 = x + width/2;
            width = w;
            height = h;
            x = x0 - w/2;
   ...
   #if (neko||cpp)
   override public function DoMouseEnter() { jsflash.Lib.SetTextCursor(true); }
   override public function DoMouseLeave() { jsflash.Lib.SetTextCursor(false); }
   ...
   override public function __Render(inMask:Dynamic,inScrollRect:Rectangle,inTX:Int,inTY:Int):Dynamic
   {
      ...
      {
         if ( (Std.int(jsflash.Lib.getTimer()*0.002) & 1) == 1 )
         {
