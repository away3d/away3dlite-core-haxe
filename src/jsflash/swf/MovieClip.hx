...
import jsflash.swf.SWF;
import jsflash.swf.Sprite;
import jsflash.swf.Frame;
...
typedef MovieClipBase = jsflash.display.MovieClip;
...
{
   var mSWF    : SWF;
   var mFrames : jsflash.swf.Frames;
         ...
         if (frame!=null)
         {
            ...
            for(depth in frame_objs.keys())
            {
               ...
               if (pool != null && pool.length > 0)
               {
                   ...
                   switch(slot.mCharacter)
                   {
                      case charSprite(sprite):
                         var clip:jsflash.swf.MovieClip = untyped disp_object;
                         clip.gotoAndPlay(1);
               ...
               {               
                   ...
                   {
                      case charSprite(sprite):
                         var movie = new jsflash.swf.MovieClip();
                      ...
                      case charShape(shape):
                         var s = new flash.display.Shape();
                          //trace( s );
                         //shape.Render(new jsflash.display.DebugGfx());
                         waiting_loader = shape.Render(s.graphics);
                         disp_object = s;

                      case charMorphShape(morph_data):
                         var morph = new jsflash.swf.MorphObject(morph_data);
                         //morph_data.Render(new jsflash.display.DebugGfx(),0.5);
                         disp_object = morph;
   ...
   public function CreateFromSWF(inSprite:jsflash.swf.Sprite)
   {
